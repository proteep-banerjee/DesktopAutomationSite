...
public class ExecutionSuite_HomePage extends Config {
	...
	public void TC_ExecutionSuite_HomePage_002_verifyDateLessSearches() {
		hp.fill_SearchBox_Txt("Karol Bagh");
		generic.goToSleep(2000);
		dp.click_firstValueFromLocationSuggestor_Lbl();
		hp.click_FindFabHotel_Btn();
		Assert.assertTrue(lp.getText_resultsCountText_Lbl().contains("Budget Hotels in and around Karol Bagh"),
				"Not redirecting to correct listing page");
	}
	...
	public void TC_ExecutionSuite_HomePage_004_verifyPopularCitiesRedirection(String cityName, String url) {
		...
		customAssert.assertEquals(driver.getCurrentUrl(), UrlProvider.getHomePageUrl() + url, "Wrong Url Formed");
		customAssert.assertTrue(lp.getText_resultsCountText_Lbl().contains("Budget Hotels in " + cityName),
	...
	public void TC_ExecutionSuite_HomePage_005_verifyMostPopularFabhotels() {
		...
		while (!driver.findElement(By.xpath(HomePage.hotelsInDemandCard_Lnk + "[last()]")).isDisplayed()) {
			for (int j = i; j < i + 2; j++) {
				...
				customAssert.assertTrue(hp.getElementText_hotelsInDemandPrices_WE(j).length() > 5, "Failed Prices");
				customAssert.assertTrue(hp.isVisible_hotelsInDemandImages_WE(j), "Failed imagess");
				String hotelName = hp.getElementText_hotelsInDemandName_Lnk(j);
				hp.click_hotelsInDemandName_Lnk(j);
				generic.switchtoNewWindow();
				customAssert.assertTrue(hotelName.equals(dp.getLabelText_hotelName_Lbl()), "Redirection Failed");
				customAssert.assertEquals(dp.getWidth_starRating_WE(), getWidth,
						"Failed width " + dp.getLabelText_hotelName_Lbl());
				customAssert.assertEquals(dp.getLinkText_reviews_Lnk(), review,
						"Failed reviews " + dp.getLabelText_hotelName_Lbl());
				driver.close();
				generic.switchtoOriginalWindow();
			}
			i = i + 2;
	...
	public void TC_ExecutionSuite_HomePage_006_verifyDealsonHomepage() {
		CustomAssert customAssert = new CustomAssert();
		customAssert.assertEquals(hp.getLabelText_fabulousDealsHeadline_Lbl(), "Fabulous deals");
		customAssert.assertEquals(hp.getLabelText_stayMoreSaveMoreHeadline_Lbl(), "Stay more, Save more");
		hp.click_fifteenPercentOffCard_Lnk();
		generic.switchtoNewWindow();
		customAssert.assertEquals(driver.getCurrentUrl(),
				UrlProvider.getHomePageUrl() + "deals/fab-grab-flat-15-percent-off");
		driver.close();
		generic.switchtoOriginalWindow();
		hp.click_rFourHundredOffCard_Lnk();
		generic.switchtoNewWindow();
		customAssert.assertEquals(driver.getCurrentUrl(), UrlProvider.getHomePageUrl() + "deals/fab-grab-rs-400-off");
		driver.close();
		generic.switchtoOriginalWindow();
		hp.click_seeAllOffers_Lnk();
		generic.switchtoNewWindow();
		generic.goToSleep(2000);
		customAssert.assertEquals(driver.getCurrentUrl(), UrlProvider.getHomePageUrl() + "deals-and-offers");
		driver.close();
		generic.switchtoOriginalWindow();
		customAssert.assertAll();
	}
	...
	public void TC_ExecutionSuite_HomePage_007_verifyReviewsOnHomepage() {
		CustomAssert customAssert = new CustomAssert();
		customAssert.assertEquals(hp.getLabelText_plusVerifiedReviewsHeadline_Lbl(), "50,000+ verified reviews");
		customAssert.assertEquals(hp.getLabelText_hearFromOurGuestsHeadline_Lbl(), "Hear from our guests");
		hp.click_readAllReviews_Lnk();
		generic.switchtoNewWindow();
		generic.goToSleep(2000);
		customAssert.assertEquals(driver.getCurrentUrl(), UrlProvider.getHomePageUrl() + "customer-reviews");
		driver.close();
		generic.switchtoOriginalWindow();
		...
		while (!driver.findElement(By.xpath(HomePage.reviewCards_Lnk + "[last()]")).isDisplayed()) {
			for (int j = i; j < i + 2; j++) {
				customAssert.assertTrue(hp.getLabelText_reviewerNameReviewCard_Lbl(j).length() > 1,
						"Failed reviewer Name");
				customAssert.assertTrue(hp.isVisible_reviewerRatingReviewCard_Lbl(j), "Failed reviewer rating");
				customAssert.assertTrue(hp.getLabelText_reviewerTextReviewCard_Lbl(j).length() > 5,
						"Failed reviewer Text");
				customAssert.assertTrue(hp.getLinkText_reviewerHotelNameReviewCard_Lnk(j).length() > 5,
						"Failed reviewer Text");
				customAssert.assertTrue(hp.getLabelText_reviewerStayDateNameReviewCard_Lbl(j).length() > 5,
						"Failed reviewer Text");
				String hotelName = hp.getLinkText_reviewerHotelNameReviewCard_Lnk(j);
				hp.click_reviewerHotelNameReviewCard_Lnk(j);
				generic.switchtoNewWindow();
				customAssert.assertTrue(hotelName.equals(dp.getLabelText_hotelName_Lbl())
						|| driver.getCurrentUrl().contains("hotels-in-hyderabad/"), "Redirection Failed");
				driver.close();
				generic.switchtoOriginalWindow();
			}
			i = i + 2;
	...
	public void TC_ExecutionSuite_HomePage_008_verifyOurHospitalityteamOnHomePage() {
		CustomAssert customAssert = new CustomAssert();
		customAssert.assertEquals(hp.getLabelText_OurHospitalityTeamHeadline_Lbl(), "Our hospitality team");
		customAssert.assertEquals(hp.getLabelText_fabPeopleFabSyatHeadline_Lbl(),
				"Fab people for a fab stay experience");
		customAssert.assertTrue(hp.getLabelText_hospitalityTeamText_Lbl().length() > 100,
				"Hopitality Team text is missing");
		customAssert.assertTrue(hp.isVisible_hospitalityTeamImage_Txt(), "Hopitality Team Image is missing");
		customAssert.assertAll();
	}
	...
	public void TC_ExecutionSuite_HomePage_009_verifyWorkWithUsOnHomePage() {
		CustomAssert customAssert = new CustomAssert();
		customAssert.assertEquals(hp.getLabelText_workWithUsHeadline_Lbl(), "Work with us");
		customAssert.assertEquals(hp.getLabelText_weAreHiringHeadline_Lbl(), "We are hiring awesome folks");
		customAssert.assertTrue(hp.getLabelText_workWithUsTeamText_Lbl().length() > 100,
				"Hopitality Team text is missing");
		customAssert.assertTrue(hp.isVisible_workWithUsImage_WE(), "Hopitality Team Image is missing");
		hp.click_seeAllOpenings_Lnk();
		generic.switchtoNewWindow();
		generic.goToSleep(2000);
		customAssert.assertEquals(driver.getCurrentUrl(), UrlProvider.getHomePageUrl() + "careers");
		driver.close();
		generic.switchtoOriginalWindow();
		customAssert.assertAll();
	}
	...
	public void TC_ExecutionSuite_HomePage_010_verifyWeAreSpokenAboutOnHomePage() {
		CustomAssert customAssert = new CustomAssert();
		customAssert.assertEquals(hp.getLabelText_spokenAboutHeadline_Lbl(), "We are being spoken about");
		customAssert.assertEquals(hp.getLabelText_readMediaCoverageHeadline_Lbl(), "Read our media coverage");
		customAssert.assertTrue(hp.isVisible_economicTimesLogo_WE(), "ET Image is missing");
		customAssert.assertTrue(hp.isVisible_forbesLogo_WE(), "Forbes Image is missing");
		customAssert.assertEquals(hp.getLabelText_seriesBfunding_Lbl(),
				"FabHotels secures $25 million Series B funding led by Goldman Sachs Investment Partners");
		customAssert.assertEquals(hp.getLabelText_expandingRapidly_Lbl(),
				"Indian Budget Hotel Chain - FabHotels - Is Expanding Rapidly");
		customAssert.assertEquals(hp.getLabelText_economicTimesDate_Lbl(), "26 July 2017 | Source : Economic Times");
		customAssert.assertEquals(hp.getLabelText_forbesDate_Lbl(), "30 Aug 2016 | Source : Forbes");
		hp.click_economicTimes_Lnk();
		generic.switchtoNewWindow();
		generic.waitForCompletePageLoad();
		generic.goToSleep(2000);
		customAssert.assertEquals(driver.getCurrentUrl(),
				"https://economictimes.indiatimes.com/small-biz/money/fabhotels-secures-25-million-series-b-funding-led-by-goldman-sachs-investment-partners/articleshow/59769274.cms");
		driver.close();
		generic.switchtoOriginalWindow();
		hp.click_forbes_Lnk();
		generic.switchtoNewWindow();
